...
class SlidingUpPanel extends React.PureComponent {
  static propTypes = {
    visibleHeightVar: PropTypes.number,
  ...
  static defaultProps = {
    ...
    draggableRange: {top: visibleHeight(100) - statusBarHeight(), bottom: 0},
    snappingPoints: [],
  ...
  constructor(props) {
    ...
    const initialValue = clamp(animatedValue, hp('100%'), top)
    console.log("this.props.visibleHeightVar",this.props.visibleHeightVar)
  ...
  _onPanResponderRelease(evt, gestureState) {
    ...
    console.log("onPanResponderRelease evt", evt.nativeEvent.locationY);
    const animatedValue = this.props.animatedValue.__getValue()
  ...
  _renderContent() {
    ...
    const animatedContainerStyles = [
      ...
      transform,
      containerStyle,
      {height: visibleHeightVar, bottom: -(visibleHeightVar)}
    ]

    if (typeof this.props.children === 'function' && this.state.visiblePanel) {
      return (
        <Animated.View
          key="content"
    ...
    } else if(this.state.visiblePanel) {
    return (
      <Animated.View
        key="content"
        pointerEvents="box-none"
